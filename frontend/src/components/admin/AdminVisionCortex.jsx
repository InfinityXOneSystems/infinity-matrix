...
const AdminVisionCortex = () => {
  ...
  const handleSend = () => {
    ...
    const newMsg = { id: Date.now(), type: 'user', content: input };
    ...
    <div className="h-full flex flex-col bg-transparent text-white overflow-hidden rounded-tl-2xl border-l-2 border-t-2 border-white/20">
      ...
      <div className="h-20 border-b-2 border-white/20 flex items-center px-8 justify-between bg-black/40 backdrop-blur-xl shrink-0">
         <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-[#0066FF]/20 rounded-xl flex items-center justify-center border-2 border-[#0066FF] shadow-[0_0_20px_rgba(0,102,255,0.4)]">
               <Cpu className="text-white" size={28} />
            </div>
            <div>
               <h1 className="font-bold text-2xl tracking-wide text-white text-glow">Vision<span className="text-[#39FF14]">Cortex</span></h1>
         ...
         <div className="flex items-center gap-6">
            <div className="hidden md:flex items-center gap-2 px-4 py-1.5 bg-black/40 rounded-lg border border-white/10">
               ...
               <span className="text-xs font-bold text-white/60"> clearance: <span className="text-white">ADMIN_L5</span></span>
            </div>
      ...
      <div className="flex-1 overflow-hidden relative flex flex-col p-6">
         <div 
            ref={scrollRef}
            className="flex-1 overflow-y-auto custom-scrollbar space-y-6 pr-4 pb-4"
         >
            {messages.map((msg) => (
              <div 
                key={msg.id} 
              ...
              >
                 <div className={cn(
                    "max-w-[85%] md:max-w-[70%] p-6 rounded-2xl border-2 backdrop-blur-md relative transition-all duration-300",
                    msg.type === 'user' 
                       ? "bg-[#0066FF]/10 border-[#0066FF]/50 text-white rounded-tr-sm hover:border-[#0066FF]" 
                       : msg.type === 'system'
                       ? "bg-transparent border-none text-white/40 font-mono text-xs w-full max-w-full text-center py-2"
                       : "bg-black/60 border-[#39FF14]/30 text-white rounded-tl-sm shadow-[0_0_15px_rgba(57,255,20,0.05)] hover:border-[#39FF14]/60"
                 )}>
                    {msg.type !== 'system' && (
                       <div className={cn(
                    ...
                    <p className={cn("leading-relaxed text-sm md:text-base", msg.type==='system' && "flex items-center justify-center gap-2")}>
         ...
         <div className="mt-6 relative shrink-0">
            <div className="absolute inset-0 bg-gradient-to-t from-[#39FF14]/5 to-transparent pointer-events-none rounded-xl" />
            <div className="glass-panel p-2 rounded-2xl flex items-center gap-4 bg-black/60 border-2 border-white/20 focus-within:border-[#39FF14]/70 focus-within:shadow-[0_0_20px_rgba(57,255,20,0.2)] transition-all">
               <div className="pl-4">
               ...
               <Button 
                  onClick={handleSend}
                  className="h-12 px-8 rounded-xl bg-[#39FF14] hover:bg-[#32e612] text-black font-bold text-lg hover:scale-105 transition-all shadow-[0_0_15px_rgba(57,255,20,0.5)]"
               >
                  <Send size={20} />
               </Button>
            </div>
